...
func newPodwithWorkingDir(namespace string, name string, containerName string, runtimeclass string) *corev1.Pod {
	servicetoken := false
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace},
		Spec: corev1.PodSpec{
			Containers:                   []corev1.Container{{Name: containerName, Image: "quay.io/sudharshanibm3/working_dir_test:latest"}},
